...
export function ThreeDayView({ tasks, onTaskClick, onTaskEdit, onTaskDelete, onTaskToggleComplete }: ThreeDayViewProps) {
    ...
    <Paper elevation={3} sx={{ p: 3 }}>
      <Box sx={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', mb: 3 }}>
        ...
        <Typography variant="h5" component="h2">
          {format(threeDays[0], 'MMM dd', { locale: enUS })} - {format(threeDays[2], 'MMM dd, yyyy', { locale: enUS })}
        </Typography>
      ...
      <Box sx={{ overflowX: 'auto' }}>
        <Grid container spacing={1}>
          ...
          {threeDays.map((day, dayIndex) => {
            ...
            return (
              <Grid item xs={11/3} key={day.toISOString()}>
                {/* Day header */}
                <Card
                  variant="outlined"
                  sx={{
